...
export default async function LeadsPage({
  ...
  searchParams: { status?: string; priority?: string; q?: string; filter?: string; deleted?: string }
}) {
  ...
  const filteredLeads = leads?.filter(lead => {
    if (searchParams.q) {
      ...
      const name = `${(lead as any).first_name || ''} ${(lead as any).last_name || ''}`.toLowerCase()
      return name.includes(q) || (lead as any).phone?.includes(q) || (lead as any).email?.toLowerCase().includes(q)
    }
    return true
  })
