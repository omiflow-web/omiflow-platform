...
export default async function DashboardPage() {
  ...
  const { data: stats } = await supabase.rpc('get_dashboard_stats', { org_id: orgId })
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {(s.urgent_leads || 0) > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <a href="/dashboard/leads?filter=urgent" className="ml-auto text-sm font-medium text-red-700 hover:text-red-900 whitespace-nowrap">View all →</a>
        </div>
      )}

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Calls Today" value={s.calls_today || 0} icon={Phone} color="bg-blue-50 text-blue-600" />
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="divide-y divide-gray-50">
            {urgentLeads && urgentLeads.length > 0 ? urgentLeads.map((lead: any) => (
              <a key={lead.id} href={`/dashboard/leads/${lead.id}`} className="flex items-center justify-between p-4 hover:bg-gray-50 rounded-lg transition-colors group">
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                  <div>
                    <div className="font-medium text-gray-900 text-sm group-hover:text-omiflow-600">
                      {[lead.first_name, lead.last_name].filter(Boolean).join(' ') || lead.phone}
                    </div>
                    <div className="text-xs text-gray-400">{lead.practice_area?.name || 'General Enquiry'}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs font-medium text-red-600">Not contacted</div>
                  <div className="text-xs text-gray-400">{lead.first_contact_at ? formatDistanceToNow(new Date(lead.first_contact_at), { addSuffix: true }) : ''}</div>
                </div>
              </a>
            )) : <div className="p-8 text-center text-gray-400 text-sm">No urgent leads — great work! 🎉</div>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm lg:col-span-2">
          ...
          <div className="divide-y divide-gray-50">
            {recentCalls && recentCalls.length > 0 ? recentCalls.map((call: any) => {
              ...
              return (
                <a key={call.id} href={`/dashboard/calls/${call.id}`} className="flex items-start gap-4 p-4 hover:bg-gray-50 transition-colors">
                  <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${call.handled_by === 'ai' ? 'bg-purple-400' : call.handled_by === 'human' ? 'bg-green-400' : 'bg-gray-300'}`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-sm font-medium text-gray-900">{call.caller_number}</span>
                      {sentiment && <span className={`text-xs px-2 py-0.5 rounded-full font-medium sentiment-${sentiment}`}>{sentiment}</span>}
                    </div>
                    {summary && <div className="text-xs text-gray-500 truncate">{summary}</div>}
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="text-xs text-gray-400">{duration}</div>
                    <div className="text-xs text-gray-400 mt-0.5">{formatDistanceToNow(new Date(call.started_at), { addSuffix: true })}</div>
                  </div>
                </a>
              )
