    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {/* Microsoft 365 */}
          <div className="flex items-center gap-2">
            {microsoftIntegration?.is_active ? (
              <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 px-3 py-2 rounded-lg">
                <CheckCircle className="w-4 h-4 text-blue-600" />
                <span className="text-xs font-medium text-blue-700">Microsoft Connected</span>
                <button onClick={() => disconnectCalendar('microsoft')} className="text-xs text-red-500 hover:underline ml-1">Disconnect</button>
              </div>
            ) : (
              <button onClick={() => connectCalendar('microsoft')} disabled={connecting === 'microsoft'}
                className="flex items-center gap-2 border border-gray-200 text-gray-600 text-sm font-medium px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50">
                {connecting === 'microsoft' ? <Loader className="w-4 h-4 animate-spin" /> : (
      ...
      {connected && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <span className="text-green-800 font-medium text-sm">
            {connected === 'google' ? 'Google Calendar' : 'Microsoft 365'} connected successfully. Appointments will now sync automatically.
          </span>
        </div>
      )}
      ...
      {oauthError && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <span className="text-red-800 text-sm">Calendar connection failed. Please try again or check your credentials.</span>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex gap-1">
              ...
              <button onClick={() => setCurrentMonth(new Date())} className="px-3 py-1 text-xs font-medium text-gray-600 hover:bg-gray-100 rounded-lg">Today</button>
            ...
            <div className="mt-5 pt-5 border-t border-gray-100">
              <h3 className="text-sm font-semibold text-gray-900 mb-3">
                ...
                {selectedDayAppointments.length > 0 && (
                  <span className="ml-2 text-xs font-normal text-gray-400">{selectedDayAppointments.length} appointment{selectedDayAppointments.length !== 1 ? 's' : ''}</span>
                )}
              </h3>
              {selectedDayAppointments.length > 0 ? (
                    ...
                    return (
                      <div key={appt.id} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500 mt-0.5">{format(parseISO(appt.starts_at), 'h:mm a')} · {appt.duration_minutes}min · {typeLabel[appt.type] || appt.type}</div>
                          <div className="text-xs text-gray-400 mt-0.5">{leadName}</div>
                        </div>
                      </div>
                    )
                  })}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            {loading ? <p className="text-sm text-gray-400">Loading...</p> : upcoming.length > 0 ? (
