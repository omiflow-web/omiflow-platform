...
export default function CalendarPage() {
  ...
  async function loadData() {
    ...
    setIntegrations(calData.integrations || [])
  }
  ...
  async function disconnectGoogle() {
    await fetch('/api/calendar', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'disconnect', provider: 'google' })
    })
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div>
          {googleIntegration ? (
            <div className="flex items-center gap-2 bg-green-50 border border-green-200 px-3 py-2 rounded-lg">
              ...
              <button onClick={disconnectGoogle} className="text-xs text-red-500 hover:underline ml-1">Disconnect</button>
            </div>
          ) : (
      ...
      {connected === 'google' && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <span className="text-green-800 font-medium text-sm">Google Calendar connected. Appointments will now sync automatically.</span>
        </div>
      )}
      ...
      {oauthError && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <span className="text-red-800 text-sm">Calendar connection failed. Please try again.</span>
        </div>
      )}
